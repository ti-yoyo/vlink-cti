...
int rc_acct(rc_handle *rh, UINT4 client_port, VALUE_PAIR *send)
{
	...
	result = ERROR_RC;
    int rand = random() % acctserver->max;
		...
		; i++)
	{
        ...
        int index = (i + rand) % acctserver->max;
		rc_buildreq(rh, &data, PW_ACCOUNTING_REQUEST, acctserver->name[index],
			    acctserver->port[index], timeout, retries);
