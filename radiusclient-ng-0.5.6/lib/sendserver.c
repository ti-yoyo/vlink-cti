	...
	for (;;)
	{
		...
		if (FD_ISSET (sockfd, &readfds))
			break;
               ...
               fds.fd = sockfd;
               fds.events = POLLIN; /* read fd data */
               fds.revents = (short)0;
               time_out = (double)data->timeout;
               poll_result = poll(&fds,1,time_out * 1000);
               if(poll_result == -1) {
               if(errno = EINTR)
                    continue;
               rc_log(LOG_ERR, "rc_send_server: select: %s", strerror(errno));
          ...
          if(poll_result == 1 && ((fds.revents & POLLIN) == POLLIN))
                  break;
		/*
