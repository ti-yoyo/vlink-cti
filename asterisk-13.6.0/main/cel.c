	...
	struct ast_channel_snapshot *new_snapshot)
{
	...
	if (!was_hungup && is_hungup) {
		...
		if(new_snapshot->manager_vars){
			struct ast_var_t *var;
			char *val;
			AST_LIST_TRAVERSE(new_snapshot->manager_vars, var, entries){
				val = ast_escape_c_alloc(var->value);
				ast_json_object_set(extra, var->name, ast_json_string_create(val));
				ast_free(val);
			}
		}
		//ast_json_object_set(extra, "ivr_id", ast_json_string_create("1"));
		ast_log(LOG_ERROR, "cel report extra=%s\n", ast_json_dump_string(extra));
