...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 314206 $")
			     ...
			     char *data, char *buf, size_t len)
{
	...
	if (!strcasecmp(data, "audionativeformat"))
		...
		ast_channel_unlock(chan);
	} else if (!strcasecmp(data, "soft_hangup")) {
		char softhangupbuf[16];
		snprintf(softhangupbuf,sizeof(softhangupbuf),"%d", chan->_softhangup&1);
		locked_copy_string(chan, buf, softhangupbuf, len);
