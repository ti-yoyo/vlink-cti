...
enum {
	...
	AST_FEATURE_WARNING_SWITCH = (1 << 9),
};
 ...
 */
struct ast_channel *__ast_request_and_dial_uniqueid(const char *type, format_t format, const struct ast_channel *requestor, void *data, 
    int timeout, int *outstate, const char *cid_num, const char *cid_name, struct outgoing_helper *oh, char *uniqueid);
