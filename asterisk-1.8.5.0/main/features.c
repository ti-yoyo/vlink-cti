...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 324652 $")
...
static int builtin_blindtransfer(struct ast_channel *chan, struct ast_channel *peer, struct ast_bridge_config *config, const char *code, int sense, void *data)
{
	...
	if (found_lot) {
		...
		if (!transferer->cdr) { /* this code should never get called (in a perfect world) */
			ast_log(LOG_DEBUG, "this code should never get called (in a perfect world)\n");
		...
		if (transferer->cdr) {
			...
			/* I don't know why to swap cdr now, maybe it is reasonable Comment By AaronAn 20120106*/
...
static int builtin_atxfer(struct ast_channel *chan, struct ast_channel *peer, struct ast_bridge_config *config, const char *code, int sense, void *data)
{
	...
	set_peers(&transferer, &transferee, peer, chan, sense);
	transferer_real_context = real_ctx(transferer, transferee);
	if(strcasecmp(transferer_real_context, "transfer") == 0){//这个是从按键来的，转化为consult
		transferer_real_context = "consult";
	}
...
/*! \brief find a dynamic feature by name reuturn NULL if not find  */
struct ast_call_feature *ast_find_dynamic_feature(const char *name)
{
    struct ast_call_feature *tmp = NULL;
...
before_you_go:
    ...
    //modify by anjb to avoid execute h twice 2014-03-11
	if (h_context && !ast_test_flag(chan, AST_FLAG_BRIDGE_HANGUP_RUN)) {
		...
		ast_channel_lock(chan);
		if (bridge_cdr) {
			/* I don't know why need swap cdr and why use bridge_cdr so just comment it. AaronAn 20120106*/
			//swapper = chan->cdr;
	...
	char *parse, struct timeval *calldurationlimit)
{
	...
	limit_var = pbx_builtin_getvar_helper(chan, "LIMIT_PLAYAUDIO_TYPE");
	if(limit_var && !strncmp(limit_var, "MIX", 3)) {	
		ast_set_flag(&(config->features_caller), AST_FEATURE_WARNING_SWITCH);
	}
