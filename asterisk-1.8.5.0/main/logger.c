...
void ast_queue_log(const char *queuename, const char *callid, const char *agent, const char *event, const char *fmt, ...)
{
	...
	//ast_log(LOG_DEBUG, "ast_queue_log %s %s %s %s\n", queuename,callid,agent,event);
	if (ast_check_realtime("queue_log")) {
		//ast_log(LOG_DEBUG, "realtime queue_log\n");
		tv = ast_tvnow();
		...
		va_end(ap);
		/*
		* anjb use manager event instead of queue_log realtime to avoid wrong queuemember status under heavy load
		*/
		...
		/* anjb add new queue_log solution using manager_event*/
		AST_DECLARE_APP_ARGS(args,
                              AST_APP_ARG(data)[5];
                );
		AST_NONSTANDARD_APP_ARGS(args, qlog_msg, '|');

                manager_event(EVENT_FLAG_AGENT, "QueueLog",
						"Time: %s\r\n"
						"CallId: %s\r\n"
                                                "QueueName: %s\r\n"
                                                "Agent: %s\r\n"
						...
						"Data4: %s\r\n"
						"Data5: %s\r\n",
                time_str, callid, queuename, agent, event,S_OR(args.data[0], ""),S_OR(args.data[1], ""),S_OR(args.data[2], ""),S_OR(args.data[3], ""),S_OR(args.data[4], ""));

		if (!logfiles.queue_log_to_file) {
			return;
		}
		//ast_log(LOG_DEBUG, "continue log to file\n");
	}
	//ast_log(LOG_DEBUG, "qlog=%x\n", qlog);
	if (qlog) {
		va_start(ap, fmt);
