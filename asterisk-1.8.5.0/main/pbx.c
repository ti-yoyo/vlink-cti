...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 324364 $")
  ...
  const char *label, const char *callerid, enum ext_match_t action, int *found, int combined_find_spawn)
{
	...
	if (e) {
		...
		if (matching_action) {
            ...
            /* comment to ignore Newexten event for performance by anjb 2013-10
...
{
    return ast_pbx_outgoing_exten_uniqueid(type, format, data, timeout, context, exten, priority, reason, synchronous, cid_num, cid_name, vars, account, channel, NULL);
}
int ast_pbx_outgoing_exten_uniqueid(const char *type, format_t format, void *data, int timeout, const char *context, const char *exten, int priority, int *reason, int synchronous, const char *cid_num, const char *cid_name, struct ast_variable *vars, const char *account, struct ast_channel **channel, char *uniqueid)
{
	struct ast_channel *chan;
	...
	if (synchronous) {
        ...
        if(uniqueid){
            chan = __ast_request_and_dial_uniqueid(type, format, NULL, data, timeout, reason, cid_num, cid_name, &oh, uniqueid);
        }else{
