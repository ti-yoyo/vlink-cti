...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 324178 $")
...
static int action_hangup(struct mansession *s, const struct message *m)
{
	...
	if ((!strcmp(cause,"1002")) && c->_bridge) {
		ast_channel_lock(c->_bridge);
		pbx_builtin_setvar_helper(c->_bridge,"cdr_force_disconnect","1");
		ast_channel_unlock(c->_bridge);
	}

	ast_channel_lock(c);
	if (!strcmp(cause,"1002")) {
		pbx_builtin_setvar_helper(c,"cdr_force_disconnect","1");
	}
	if (causecode > 0) {
...
/* add by AaronAn 2011-09-01 */
static int action_indicate(struct mansession *s, const struct message *m)
{
    const char *name = astman_get_header(m, "Channel");
	const char *indicate = astman_get_header(m, "Indicate");
	int control_code;
	if (indicate && sscanf(indicate, "%30d", &control_code) != 1){
        astman_send_error(s, m, "No indicate specified");
        return 0;
	}
    ...
    if(!(chan = ast_channel_get_by_name(name))) {
        astman_send_error(s, m,"No such channle");
        return 0;
    }

    ast_channel_lock(chan);
    ast_indicate(chan, control_code);
    if(chan->_bridge)
        ast_indicate(chan->_bridge, control_code);
    ast_channel_unlock(chan);
    chan = ast_channel_unref(chan);
    astman_send_ack( s, m, "success");
    return 0;
...
static int action_feature(struct mansession *s, const struct message *m) {
    const char *name = astman_get_header(m, "Channel");
    const char *exten = astman_get_header(m, "Extension");
    const char *context = astman_get_header(m,"Context");
	const char *feature = astman_get_header(m,"Feature");

    struct ast_channel *chan = NULL;
    struct ast_call_feature *transfer_feature = NULL;
    char *feature_code = NULL;

    ast_debug(1,"channel name %s   exten : %s   context %s\n", name, exten, context);
    if(ast_strlen_zero(name)) {
        astman_send_error(s, m, "No channel specified");
        return 0;
    }
    ...
    if(ast_strlen_zero(feature)) {
        astman_send_error(s, m, "No feature specified");
        return 0;
    }

    if(!(transfer_feature = ast_find_call_feature(feature))) {
        astman_send_error(s, m, "No such feature feature found");
        return 0;
    }

    if(!(chan = ast_channel_get_by_name(name))) {
        astman_send_error(s, m, "No found channel");
        return 0;
    }
    ...
    ast_debug(1,"feature code %s    \n",transfer_feature->exten);
    for(feature_code = transfer_feature->exten; feature_code && *feature_code;
        ++feature_code) {
...
static int action_dynamic_feature(struct mansession *s, const struct message *m)
{
    const char *name = astman_get_header(m, "Channel");
	const char *dynamic = astman_get_header(m, "Dynamic");

    struct ast_channel *chan = NULL;
    struct ast_call_feature *initfeature = NULL;
    char *feature_code = NULL;
    ...
    if(ast_strlen_zero(dynamic)) {
        astman_send_error(s, m, "No dynamic specified");
        return 0;
    }
    ...
    if(!(initfeature = ast_find_dynamic_feature(dynamic))) {
        astman_send_error(s, m, "No such dynamic feature found");
        return 0;
    }

    for (feature_code = initfeature->exten; feature_code && *feature_code; ++feature_code) {
        struct ast_frame f = { AST_FRAME_DTMF, .subclass.integer = *feature_code };
        ast_queue_frame(chan, &f);
    }
...
static int action_originate(struct mansession *s, const struct message *m)
{
	...
	*data++ = '\0';
	...
	if (ast_true(async)) {
		...
		if (exten && context && pi) {
			res = ast_pbx_outgoing_exten_uniqueid(tech, format, data, to, context, exten, pi, &reason, 1, l, n, vars, account, NULL, uniqueid);
		} else {
	...
	ast_channel **chans, const char *file, int line, const char *func, const char *fmt, ...)
{
	...
	if (sessions) {
		...
		while ((session = ao2_iterator_next(&i))) {
			ao2_lock(session);
            if(session->readperm != 0){//add by anjb for performance
...
static int __init_manager(int reload)
{
	...
	if (!registered) {
        ...
        /* add by AaronAn 2011-09-01 */
        ast_manager_register_xml("Indicate", EVENT_FLAG_CALL, action_indicate);
		ast_manager_register_xml("Feature",EVENT_FLAG_CALL, action_feature);
		ast_manager_register_xml("Dynamic", EVENT_FLAG_CALL, action_dynamic_feature);
        /* AaronAn add end */
