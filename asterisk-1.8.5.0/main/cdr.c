...
static void post_cdr(struct ast_cdr *cdr)
{
	...
	for ( ; cdr ; cdr = cdr->next) {
		...
		//ast_log(LOG_NOTICE, "src %s\n",cdr->src);
		//ast_log(LOG_NOTICE, "dst %s\n",cdr->dst);
		//ast_log(LOG_NOTICE, "dcontext %s\n",cdr->dcontext);
		//ast_log(LOG_NOTICE, "channl %s\n",cdr->channel);
		//ast_log(LOG_NOTICE, "dstchannel %s\n",cdr->dstchannel);
		//ast_log(LOG_NOTICE, "lastapp %s\n",cdr->lastapp);
		//ast_log(LOG_NOTICE, "lastdata %s\n",cdr->lastdata);
		if (ast_localtime(&cdr->start, &tm, NULL)) {
			ast_strftime(buf, sizeof(buf), "%H:%M:%S", &tm);
			//ast_log(LOG_NOTICE, "start %d\n", cdr->start.tv_sec);
		}
        if (ast_localtime(&cdr->answer, &tm, NULL)) {
            ast_strftime(buf, sizeof(buf), "%H:%M:%S", &tm);
            //ast_log(LOG_NOTICE, "answer %d\n", cdr->answer.tv_sec);
        }
        if (ast_localtime(&cdr->end, &tm, NULL)) {
            ast_strftime(buf, sizeof(buf), "%H:%M:%S", &tm);
            //ast_log(LOG_NOTICE, "end %d\n", cdr->end.tv_sec);
        }
		//ast_log(LOG_NOTICE, "duration %ld\n",cdr->duration);
		//ast_log(LOG_NOTICE, "billsec %ld\n",cdr->billsec);
		//ast_log(LOG_NOTICE, "disposition %ld\n",cdr->disposition);
		//ast_log(LOG_NOTICE, "amaflags %s\n",ast_cdr_flags2str(cdr->amaflags));
		//ast_log(LOG_NOTICE, "flags %d\n",cdr->flags);
		//ast_log(LOG_NOTICE, "uniqueid %s\n",cdr->uniqueid);
		//ast_log(LOG_NOTICE, "linkedid %s\n",cdr->linkedid);
		//ast_log(LOG_NOTICE, "userfield %s\n",cdr->userfield);
		//ast_log(LOG_NOTICE, "sequence %d\n",cdr->sequence);
		struct ast_var_t *variables;
		struct varshead *headp = &cdr->varshead;
		AST_LIST_TRAVERSE(headp, variables, entries) {
