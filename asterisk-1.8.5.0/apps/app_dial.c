...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 325537 $")
...
static int dial_exec_full(struct ast_channel *chan, const char *data, struct ast_flags64 *peerflags, int *continue_exec)
{
	...
	if (!peer) {
		...
		if (chan->cdr) {
			...
			//comment by AaronAn 20120116 since we maintain the cdr ourself
			//ast_log(LOG_DEBUG, ">>>>>>>>>>> reset cdr answer to peer in app_dial.c\n");
		...
		if (!ast_test_flag64(&opts, OPT_ANNOUNCE) || ast_strlen_zero(opt_args[OPT_ARG_ANNOUNCE])) {
			...
			while (!res && (streamfilename = strsep(&streamfileargs, "&"))) {
				...
				res = ast_streamfile(peer, streamfilename, peer->language);
				if (res) {
					res = 0;
