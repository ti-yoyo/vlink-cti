...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 325614 $")
...
struct queue_ent {
	...
	char dialed[100][80];					/*!< The dialed member max 100 added by anjb 20140728 */
};
...
{	
	//if(m->status != status){
	//ast_log(LOG_DEBUG, "%s old status=%d new=%d\n",m->membername, m->status, status);
		m->status = status;
	...
	if (paused_str) {
 	...
 	while ((m = ao2_iterator_next(&mem_iter))) {
 		if (!strcasecmp(m->rt_uniqueid, rt_uniqueid)) {
 			...
 			//如果找到座席数据，不更新paused，只有第一次登陆时使用paused，解决置忙登录可能立刻接到电话问题 by anjb 2013-1104
            //if (paused_str)
 			//	m->paused = paused;
			...
			/*add by lidp for synchronization of membername */
			if(strcasecmp(membername, m->membername)){
               ast_log(LOG_DEBUG,"===  membername = %s\n",m->membername);
               ast_copy_string(m->membername, membername, sizeof(m->membername));
               ast_log(LOG_DEBUG,"=== test after update membername = %s\n",m->membername);
            }
            /* lidp add */
            if(strcasecmp(interface, m->interface)){
               ast_log(LOG_DEBUG,"=== test before  update interface = %s\n",m->interface);
               ast_copy_string(m->interface, interface, sizeof(m->interface));
               ast_log(LOG_DEBUG,"=== test after update interface = %s\n",m->interface);
            }
            /*lidp end */
	...
	ao2_iterator_destroy(&mem_iter);
 	...
 	if (!found) {
		if ((m = create_queue_member(interface, membername, penalty, paused, state_interface))) {
            //增加平均、最长空闲时间策略时退出座席可以记住calls与lastcall by anjb 2013-11-18
            char member_calls_value[64];
            if(ast_db_get("MEMBER_CALLS", m->membername, member_calls_value, sizeof(member_calls_value)) == 0){
                if(strlen(member_calls_value) > 0){
                    m->calls = atoi(member_calls_value);
                }
            }
            char member_lastcall_value[64];
            if(ast_db_get("MEMBER_LASTCALL", m->membername, member_lastcall_value, sizeof(member_lastcall_value)) == 0){
                if(strlen(member_lastcall_value) > 0){
                    m->lastcall = atol(member_lastcall_value);
                }
            }
...
static struct call_queue *find_queue_by_name_rt(const char *queuename, struct ast_variable *queue_vars, struct ast_config *member_config)
{
	...
	if (!queue_vars) {
		...
		if (q) {
			...
			/* Delete by anjb 2013-12-12 for avoid deadlock */
	...
	while ((m = ao2_iterator_next(&mem_iter))) {
		if (m->dead) {
            ...
            if(m->calls > 0){
                char member_calls_value[64];
                snprintf(member_calls_value, sizeof(member_calls_value), "%d", m->calls);
                char member_lastcall_value[64];
                snprintf(member_lastcall_value, sizeof(member_lastcall_value), "%ld", m->lastcall);
                ast_db_put("MEMBER_CALLS", m->membername, member_calls_value);
                ast_db_put("MEMBER_LASTCALL", m->membername, member_lastcall_value);
            }
		}
		ao2_ref(m, -1);
	}
...
static void leave_queue(struct queue_ent *qe)
{
	...
	if (q->dead) {	
		...
		//don't unlink queue anymore to avoid destroy queue by bad realtime by anjb 2014-08-12
...
static int ring_entry(struct queue_ent *qe, struct callattempt *tmp, int *busies)
{
	...
	if ((res = ast_call(tmp->chan, location, 0))) {
		...
		//Add by wuchengxian 2014-02-36 ; if did not send invite to the gateway , did not write the CDR
        ast_channel_lock(tmp->chan);
		if(tmp->chan->hangupcause ==  AST_CAUSE_USER_BUSY){
			if(tmp->chan->cdr){
				ast_log(LOG_ERROR,"Out call , peer busy OR no audio code OR sip network error ! In this case is  AST_CAUSE_USER_BUSY , peer is : %s \n",tmp->interface);
				ast_cdr_free(tmp->chan->cdr);
				tmp->chan->cdr=NULL;
			}
		}
        ast_channel_unlock(tmp->chan);
	...
	//app_queue add cdr status
	ast_channel_lock(qe->chan);
    const char *cdr_call_type = pbx_builtin_getvar_helper(qe->chan, "cdr_call_type");
    if(cdr_call_type){
        if(!strcasecmp(cdr_call_type, "1") || !strcasecmp(cdr_call_type, "2") ) {//ib or web400ib
            const char *cdr_status = pbx_builtin_getvar_helper(qe->chan, "cdr_status");
            if(cdr_status){
                if(!strcasecmp(cdr_status, "3")) {//system_answer
                    pbx_builtin_setvar_helper(qe->chan, "cdr_status", "2");
                }
            }
        }
    }
	...
	/* added by anjb 20140728 */
	int dialed_count = 0;
    int i=0;
    for(i=0; i< 100; i++){
        if(!ast_strlen_zero(qe->dialed[i])){
            dialed_count++;
        }else{
            break;
        }
   	...
   	if(dialed_count < 99){
       strncpy(qe->dialed[dialed_count], tmp->member->membername, 79);
   	}
    pbx_builtin_setvar_helper(qe->chan, "cdr_bridged_cno", cno);
	update_status(qe->parent, tmp->member, get_queue_member_status(tmp->member));
	return 1;
}
...
/*! \brief find the entry with the best metric, or NULL */
static struct callattempt *find_remember(struct callattempt *outgoing, const char *queue_remember_member)
{
    if(!queue_remember_member){
        return NULL;
    }
	struct callattempt *remember = NULL, *cur;

	for (cur = outgoing; cur; cur = cur->q_next) {
		if (cur->stillgoing &&					/* Not already done */
			!cur->chan &&					/* Isn't already going */
			!strcasecmp(cur->member->membername, queue_remember_member)) {		/* We haven't found one yet, or it's better */
			remember = cur;
                    break;
		}
	}
...
static int ring_one(struct queue_ent *qe, struct callattempt *outgoing, int *busies)
{
	...
	while (ret == 0) {
		...
		pbx_builtin_setvar_helper(qe->chan, "var_cdr_detail_mark", "");//首先置空，防止呼叫非记忆座席继承变量
        const char *queue_remember_member;
		queue_remember_member = pbx_builtin_getvar_helper(qe->chan, "queue_remember_member");
		if (queue_remember_member) {
			best = find_remember(outgoing, queue_remember_member);
			pbx_builtin_setvar_helper(qe->chan, "queue_remember_member", "");
			pbx_builtin_setvar_helper(qe->chan, "var_cdr_detail_mark", "1");
		}
		if(!best){
					...
					if (f->frametype == AST_FRAME_CONTROL) {
						...
						case AST_CONTROL_BUSY:
							...
							if (qe->parent->strategy != QUEUE_STRATEGY_RINGALL) {
								...
								if (*to > 500) {
									//Aaron add safe sleep 1 second 2013-01-15
									ast_safe_sleep(qe->chan, 1 * 1000);
									ring_one(qe, outgoing, &numbusies);
									starttime = (long) time(NULL);
								}
							}
...
static int calc_metric(struct call_queue *q, struct member *mem, int pos, struct queue_ent *qe, struct callattempt *tmp)
{
	...
	case QUEUE_STRATEGY_RRORDERED:
        for(i=0; i< 100; i++){
            if(!ast_strlen_zero(qe->dialed[i])){
                if(!strcmp(qe->dialed[i], tmp->member->membername)){
                    dialed++;
                }
            }else{
                break;
        ...
        if(dialed > 0){
            tmp->metric = dialed * 10000000 + ast_random() % 100;
        } else {
            tmp->metric = ast_random() % 100;
        }
        tmp->metric += mem->penalty * 100 * usepenalty;
        break;
	...
	case QUEUE_STRATEGY_FEWESTCALLS:
        ...
        if(dialed > 0){
            tmp->metric = dialed * 10000000 + mem->calls;
        }else{
            tmp->metric = mem->calls;
        }
        tmp->metric += mem->penalty * 1000 * usepenalty;
        break;
...
static int try_calling(struct queue_ent *qe, const char *options, char *announceoverride, const char *url, int *tries, int *noption, const char *agi, const char *macro, const char *gosub, int ringing)
{
	...
	*/
	...
	if (qe->parent->strategy == QUEUE_STRATEGY_LINEAR || qe->parent->strategy == QUEUE_STRATEGY_RRORDERED) {
	...
	if (!peer) {
		...
		ao2_lock(qe->parent);
		callcompletedlast = now - qe->start;
		...
		qe->handled++;
		ast_queue_log(queuename, qe->chan->uniqueid, member->membername, "CONNECT", "%ld|%s|%ld", callcompletedlast, peer->uniqueid,
...
static int set_member_paused(const char *queuename, const char *interface, const char *reason, int paused)
{
    ...
    //PAUSEALL与UNPAUSEALL事件没有使用，所以注释掉，提高性能
	//if (ast_strlen_zero(queuename))
	//	ast_queue_log("NONE", "NONE", interface, (paused ? "PAUSEALL" : "UNPAUSEALL"), "%s", S_OR(reason, ""));
		...
		if (ast_strlen_zero(queuename) || !strcasecmp(q->name, queuename)) {
			if ((mem = interface_exists(q, interface))) {
				...
				if (queue_persistent_members)
					dump_queue_members(q);
                if(already_send == 0){
                    ast_queue_log("NONE", "NONE", mem->membername, (paused ? "PAUSE" : "UNPAUSE"), "%s", S_OR(reason, ""));
                    already_send = 1;
                }
...
static int ql_exec(struct ast_channel *chan, const char *data)
{
	...
	ast_log(LOG_NOTICE, "QL-Queuename:%s - membername:%s - event:%s\n", args.queuename, args.membername, args.event); // liucl add 20140528
	return 0;
}
...
static int queue_exec(struct ast_channel *chan, const char *data)
{
	...
	const char *queue_first_play_file;
	int prio;
	int qcontinue = 0;
	...
	queue_first_play_file = pbx_builtin_getvar_helper(chan, "queue_first_play_file");
	...
	//anjb add 2014-07-28 init qe.dialed
    memset(qe.dialed, 0, 100*80);  
check_turns:
	if (ringing) {
		ast_indicate(chan, AST_CONTROL_RINGING);
	} else {
		ast_moh_start_first(chan, qe.moh, queue_first_play_file);
	}
...
static char *__queues_show(struct mansession *s, int fd, int argc, const char * const *argv)
{
	...
	while ((q = ao2_t_iterator_next(&queue_iter, "Iterate through queues"))) {
		...
		/* Aaron An delete 2011-09-01 begin */
		/*
		...
		*/
		/* Aaron An delete 2011-09-01 end */
