...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 323154 $")
...
static enum agi_result run_agi(struct ast_channel *chan, char *request, AGI *agi, int pid, int *status, int dead, int argc, char *argv[])
{
    ...
    //anjb add for enable timeout for AGI
	...
	for (;;) {
        if(time(NULL) - connect_time >= 10){//default 10s to AGI timeout
            ast_log(LOG_ERROR, "timeout of agi\n");
            break;
        }
		if (needhup) {
			needhup = 0;
		...
		ms = 10000;//default 10s to wait timeout
		if (dead) {
