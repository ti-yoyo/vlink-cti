...
struct mohclass {
	...
	char **filearray;
	/* 1 if moh will play first only once. by anjb 2014-10-24*/
	int first_once;
...
static int ast_moh_files_next(struct ast_channel *chan) 
{
	...
	int skip_pos = -1;
	if(state->class->first_once == 1){
		skip_pos = state->class->total_files - 1;//first_once=1时，第一个文件实际被放到filearray最后一个
	}
	if (state->pos == 0 && state->save_pos_filename == NULL) {
		if(state->class->first_once == 1){//如果是第一次进来的先播放filearray最后一个文件也就是我们前面放进去的first_file
			state->pos = state->class->total_files - 1;
			skip_pos = -1;//第一次进来必须取消skip_pos，让播放最后一个
		}
	...
	for (tries = 0; tries < state->class->total_files; ++tries) {
		if(state->pos == skip_pos){//如果应该被过滤就继续，对应多次播放等待音乐，first_file属于skip，后面不再播放
			state->pos++;
			state->pos %= state->class->total_files;
			continue;
		}
...
static int local_ast_moh_start_first(struct ast_channel *chan, const char *mclass, const char *interpclass, const char *first_file)
{
	struct mohclass *mohclass = NULL;
	struct moh_files_state *state = chan->music_state;
	...
	/* 根据first_file判断，如果设置了，那么加入到mohclass中*/
	if(!ast_strlen_zero(first_file)){
		mohclass->first_once = 1;
		moh_add_file(mohclass, first_file);
	}else{
		mohclass->first_once = 0;
	}
...
{
	return local_ast_moh_start_first(chan, mclass, interpclass, NULL);
}
...
static int load_module(void)
{
	...
	if (!load_moh_classes(0) && ast_check_realtime("musiconhold") == 0) { 	/* No music classes configured, so skip it */
		...
		ast_install_music_functions(local_ast_moh_start, local_ast_moh_start_first, local_ast_moh_stop,
